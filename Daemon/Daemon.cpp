...
map<string, string> readIniFile(const string& filePath) {
    ...
    if (!file.is_open()) {
        // Запись ошибки в системный журнал
...
void initialize() {
    ...
    //           Инициализируйте переменные isDaemonRunning и pidFilePath здесь.
...
void savePIDToFile() {
    ...
    if (pidFile.is_open()) {
        pidFile << getpid(); // getpid() возвращает PID текущего процесса
        pidFile.close();
    }
}
